...
class PointCloudUtils:
    def downsample_pcd(
        self, pcd_arr, input_file_format, pnet=True, downsample_amt=0.05
    ):
        # if pnet:
        #     pcd_format = "pnet"
        # else:
    ...
    def ds_npy(self, pcd_arr, downsample_amt=0.05):
        ...
        rgb = np.hstack(
            (truth_label, np.zeros((pcloud_shape, 1)), np.zeros((pcloud_shape, 1)))
        )  # form a 3D vector to add to o3d pcd
        normals = np.hstack(
            (intensity, np.zeros((pcloud_shape, 1)), np.zeros((pcloud_shape, 1)))
        )  # form a 3D vector to add to o3d pcd
        ...
        np_pcloud = np.hstack(
            (
                (np.asarray(pcd_alt.points)),
                (np.asarray(pcd_alt.colors)),
                (np.asarray(pcd_alt.normals)),
            )
        )
        ...
        print(
            "Original Num Points:",
            npoints,
            "\nDownsampled Num Points:",
            ndownpoints,
            "\nNew is",
            (100 - ((ndownpoints / npoints) * 100)),
            "% smaller",
        )
