...
class DataLoader:
    ...
    def __init__(
        ...
        block_size=1.0,
        padding=0.001,
        path="./Data/church_registered_pnet.ply",
    ):
        ...
        data = np.hstack(
            (np.asarray(pcd.points), np.asarray(pcd.colors), np.asarray(pcd.normals))
        )
    ...
    def __getitem__(self, index):
        ...
        for index_y in range(0, grid_y):
            ...
            for index_x in range(0, grid_x):
                ...
                normlized_xyz = np.zeros(
                    (point_size, 3)
                )  # X,Y,Z VALUES CENTERED ON THE ORIGIN
                ...
                )  # normalized - if just take point indexes - also return orginal points - FIRST THREE COLUMNS OF DATA BATCH BEFORE NORMALIZED
                label_room = (
