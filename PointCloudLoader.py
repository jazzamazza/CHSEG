...
class PointCloudLoader:
  ...
  # def get_cloud_compare_truth(self, vis, downsample=False, ds_size=0):
  #   self = "/Users/A102178/Desktop/data/church_registered.npy"
  #   self1 = "/Users/A102178/Desktop/data/church_registered _cloudCompare.las"
  #   final_pcd_npy, final_pcd_npy_truth = PointCloudLoader.load_point_cloud_npy(self, vis, downsample=False, ds_size=0)
  #   final_pcd_cloud = PointCloudLoader.load_point_cloud_las(self1, vis, downsample=False, ds_size=0)

  #   combined_truth = np.hstack(final_pcd_cloud, final_pcd_npy_truth[:,4:5])
  #   print("combined_truth", combined_truth)
  ...
  def load_point_cloud_las(self, vis, downsample=False, ds_size=0):
      ...
      #also load regular npy
      #downsample both
      #append ground truths to cloud compare and pass through   

      npy_path = "/Users/A102178/Desktop/data/church_registered.npy"
   
      pcd_npy = np.load(npy_path)         #npy file 
      print("original pcd[0]:",pcd_npy[0])
      points_npy = pcd_npy[:,:3]
      print("points[0] for npy",points_npy[0])
      intensity_npy = pcd_npy[:,3:4]
      print("intensity[0]",intensity_npy[0])
      truth_label_npy = pcd_npy[:,4:5]
      print("truth label[0]",truth_label_npy[0]) 

      final_pcd_npy = np.hstack((points_npy, intensity_npy)) #without truth label
      final_pcd_all_npy = np.hstack((points_npy, intensity_npy, truth_label_npy))
      self.get_attributes(final_pcd_npy, "final_pcd_npy") 
      print("hstacked pcd_npy[0]:",final_pcd_npy[0])

      if downsample:
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(pcd_npy[:,:3])
        rawFeatures = np.hstack((intensity_npy, truth_label_npy, truth_label_npy))
        pcd.normals = o3d.utility.Vector3dVector(rawFeatures)
        downpcd = pcd.voxel_down_sample(voxel_size=ds_size) # downsample pcd
        pc_points = np.asarray(downpcd.points) # convert pcd points to np array
        pc_features = np.asarray(downpcd.normals) # convert pcd additional features to np array
        ...
        print("Downsampled Point cloud size: ", pc.size)
        down_finalPCD = np.delete(pc, [4,5], 1) # remove info unneccessary for clustering from pcd
        final_pcd_all_npy = pc
        print("ground truth in pc:", final_pcd_all_npy[:,4:5])
        self.get_attributes(down_finalPCD, "final_pcd") 
        print(down_finalPCD[0])
        final_pcd_npy = down_finalPCD
      ...
      pcd = lp.read(path) 
      print("pcd cloud compare original points", pcd[0], pcd[1], pcd[2]) #same as the points from the npy
      ...
      if downsample:
            ...
            print("downsampled cloud compare yes")
      else:
      ...
      final_combined = np.hstack((final_pcd, final_pcd_all_npy[:,4:5]))
      print("final_combined:", final_combined[0])

      #save cloud compare file
      np.save("/Users/A102178/Desktop/data/0.085DS_Cloud_compare.las", final_pcd)
